...
Pago::Pago(void)
{
    for (unsigned int i = 0; i < sizeof(id); id[i++] = '\0');
    for (unsigned int i = 0; i < sizeof(tarjeta); tarjeta[i++] = '\0');
    for (unsigned int i = 0; i < sizeof(fecha); fecha[i++] = '\0');
    for (unsigned int i = 0; i < sizeof(cantidad); cantidad[i++] = '\0');
}
...
void Pago::genera(void)
{
    Pago pago;
    int contador = 0;
    ofstream archivo("Payments.txt", ios::out);
    if (!archivo)
